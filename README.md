...
Your browser opens `/devrig/setup` — a status dashboard that updates live when Claude connects. `CLAUDE.md` (generated by `devrig init`) tells Claude to open `localhost:3000` in its Chrome MCP tab group, and the setup page flips to "Connected." On the first launch after a fresh install, Chrome MCP isn't available yet — Claude will tell you to `/exit` and `devrig start` again to activate it.
